 ...
 (function($) {
	...
	$.fn.clockTimePicker = function(options, _value) {
		...
		************************************************************************************************/
		...
		if (isitaMobile()) css += ' .clock-timepicker input::selection { background:rgba(255,255,255,0.6); } .clock-timepicker input::-moz-selection { background:rgba(255,255,255,0.6); }';
		...
		return this.each(function() {
			...
			if (isitaMobile()) element.prop('readonly', true);
			var oldValue = element.val();
			var enteredDigits = '';
			var selectionMode = 'HOUR'; //2 modes: 'HOUR' or 'MINUTE'
			var isDragging = false;
			var touchSignButton = false;
			var popupWidth = isitaMobile() ? $(document).width() - 80 : settings.popupWidthOnDesktop;
			var canvasSize = popupWidth - (isitaMobile() ? 50 : 20);
			...
			canvasHolder.css('position', 'relative')
						...
						.css('margin', '10px ' + (isitaMobile() ? 25 : 10) + 'px');
			...
			if (isitaMobile()) {
				...
				var buttonHtml = 'x<br><a style="text-decoration:none; color:' + settings.colors.buttonTextColor + '; font-family:' + settings.fonts.fontFamily + '; font-size:' + settings.fonts.buttonFontSize + 'px; padding-left:30px">';
				...
				okButton.on('click', function() {
					if (isitaMobile()) element.val(getInputElementValue());
			...
			function showTimePicker() {
				...
				if (!isitaMobile() && settings.onlyShowClockOnMobile) popup.css('visibility', 'hidden');
				if (isitaMobile()) adjustMobilePopupDimensionAndPosition();
			 ...
			 ************************************************************************************************/
			function autosize() {
				if (!settings.autosize || isitaMobile()) return;
